window.onload = function() {
  ...
  function generateRows(sRows) {
    ...
    for (var i=0;i<sRows.length;i++){
      ...
      row.innerHTML+=`<td><a href="/v/${sRows[i][2]['_id']}">${sRows[i][2]['pollName']}</a></td> <td>${sRows[i][2]['totalVotes']}</td> <td>${sRows[i][2]['accessCt']}</td>`;
      row.innerHTML+=`<td>${expiryText}</td> <td><span id="delete"><a href="/d/${sRows[i][2]['_id']}">delete</a></span></td></tr>`;
